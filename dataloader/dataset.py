...
class LungNoduleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # # 添加channel维度 [D, C=1, H, W]
        # T0_image = torch.tensor(T0_image, dtype=torch.float32).unsqueeze(1) 
        # T1_image = torch.tensor(T1_image, dtype=torch.float32).unsqueeze(1)
        # T2_image = torch.tensor(T2_image, dtype=torch.float32).unsqueeze(1)


        # 添加channel维度 [C=1, D, H, W]
        T0_image = torch.tensor(T0_image, dtype=torch.float32).unsqueeze(0) 
